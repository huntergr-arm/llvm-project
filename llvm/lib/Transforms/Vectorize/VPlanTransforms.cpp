...
void VPlanTransforms::convertCSARecurrences(VPlan &Plan,
  ...
  for (const auto &[Phi, RdxDesc] : Legal->getReductionVars()) {
    if (RecurrenceDescriptor::isCSARecurrenceKind(
      ...
      for (VPUser *U : DataSelect->users()) {
        ...
        // For RVV, extra select?
        if (I && I->getOpcode() == Instruction::Select) {
          // Hack: Assume single outside user.
          VPUser *U = I->Users[0];
          VPInstruction *Extract = cast<VPInstruction>(U);
          assert(Extract->getOpcode() == VPInstruction::ExtractLastActive &&
                 "Wrong user!");
          Extract->addOperand(MaskSelect);
          Extract->addOperand(Default);
        }
      }
    }
  }
